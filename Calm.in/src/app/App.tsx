...
export default function App() {
  ...
  const [agendas, setAgendas] = useState([
    { id: 1, title: 'Sesi Dr. Amanda L.', time: '14:00', type: 'session', date: 14 },
    { id: 2, title: 'Meditasi Pernapasan', time: '19:30', type: 'personal', date: 14 },
    { id: 3, title: 'Jurnal Malam', time: '21:00', type: 'personal', date: 14 },
    { id: 4, title: 'Jalan Pagi', time: '06:30', type: 'personal', date: 15 },
    { id: 5, title: 'Sesi Dr. Budi S.', time: '10:00', type: 'session', date: 12 }, // Selesai
  ]);
  ...
  const features = [
    ...
    { id: 'meditate', icon: Wind, title: 'Meditasi', desc: 'Relaksasi pikiran', action: () => {} },
  ];
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-slate-100 dark:bg-slate-900 flex items-center justify-center font-sans p-4 sm:p-8">
        {/* Mobile Device Mockup Container */}
        <div className="w-full max-w-[400px] bg-background h-[100dvh] sm:h-[850px] sm:max-h-[90vh] sm:rounded-[3rem] relative shadow-2xl overflow-hidden flex flex-col ring-8 ring-slate-800/90 dark:ring-slate-950">
          ...
          <header className="px-6 pt-12 pb-4 flex justify-between items-center bg-background/95 backdrop-blur-md sticky top-0 z-10 border-b border-border/50">
              ...
              <>
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-extrabold text-xl tracking-tight text-primary">Calm.in</span>
                  </div>
                  <h1 className="text-xl font-bold text-foreground tracking-tight">Halo, Nabil 👋</h1>
                  <p className="text-sm text-muted-foreground mt-1">Semoga harimu menyenangkan</p>
                </motion.div>
                <motion.button 
                  initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.1 }}
                  className="relative p-2.5 bg-secondary rounded-full text-primary hover:bg-primary hover:text-white transition-colors self-start mt-2"
                >
                  <Bell size={20} />
                  <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 border-secondary"></span>
                </motion.button>
              </>
            )}
            ...
            {currentTab === 'schedule' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 text-center">
                <h1 className="text-xl font-bold text-foreground tracking-tight">Jadwal Kalender</h1>
              </motion.div>
            )}
            ...
            {currentTab === 'profile' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex w-full justify-between items-center">
                ...
                <button className="p-2 bg-secondary text-primary rounded-full"><Settings size={20} /></button>
              </motion.div>
            )}
          </header>
            ...
            <AnimatePresence mode="wait">
                    ...
                    <div className="bg-primary text-primary-foreground rounded-3xl p-6 relative overflow-hidden shadow-xl shadow-primary/20">
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="p-2.5 bg-white/10 text-white rounded-full hover:bg-white hover:text-primary transition-colors backdrop-blur-md">
                      ...
                      <div key={post.id} className="mb-4 bg-background border-b border-border shadow-sm pb-4">
                        ...
                        <div className="p-4 pb-2 flex justify-between items-center">
                          <div className="flex gap-4">
                            ...
                            <button className="text-foreground"><MessageCircle size={26} /></button>
                            <button className="text-foreground"><Send size={26} /></button>
                          </div>
                          <button className="text-foreground"><Bookmark size={26} /></button>
                        </div>
                        {/* Caption */}
                        <div className="px-4">
              ...
              {currentTab === 'schedule' && (
                ...
                >
                  ...
                  <div className="bg-background px-6 py-4 border-b border-border shadow-sm sticky top-0 z-10">
                    <div className="flex justify-between items-center mb-6">
                      <h2 className="font-bold text-foreground text-lg">Oktober 2023</h2>
                    ...
                    {/* Week row */}
                    <div className="flex justify-between items-center">
                       {[{d: 12, day: 'Sen'}, {d: 13, day: 'Sel'}, {d: 14, day: 'Rab'}, {d: 15, day: 'Kam'}, {d: 16, day: 'Jum'}, {d: 17, day: 'Sab'}, {d: 18, day: 'Min'}].map(item => (
                         <button 
                           key={item.d}
              ...
              {currentTab === 'profile' && (
                ...
                >
                  <div className="relative">
                    <img 
                      src="https://media.licdn.com/dms/image/v2/D5603AQGkw8PEXsyKIg/profile-displayphoto-scale_200_200/B56Z4GHT2sGsAY-/0/1778219042934?e=2147483647&v=beta&t=5XXJTNbXTYQYdL1fsGbnNKdnjY_NIocwq3QEukS1AB4" 
                      alt="Profile" 
                      className="w-28 h-28 rounded-full border-4 border-background shadow-xl object-cover ring-4 ring-secondary" 
                    />
                    <button className="absolute bottom-0 right-0 w-8 h-8 bg-primary text-white rounded-full flex items-center justify-center shadow-lg border-2 border-background">
                      <PenTool size={14} />
                    </button>
                  </div>
                  ...
                  <div className="w-full bg-card border border-border rounded-3xl overflow-hidden shadow-sm">
                    {[
                      ...
                      <button key={i} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors border-b border-border last:border-0">
                  ...
                  <button className="w-full py-4 text-red-500 font-bold flex items-center justify-center gap-2 bg-red-50 rounded-2xl hover:bg-red-100 transition-colors">
            ...
            >
                ...
                <div className="p-6 pt-10 flex flex-col items-center text-center">
                  {!isCalling ? (
                    ...
                    <div className="py-8 flex flex-col items-center w-full">
                      ...
                      <button onClick={() => { setActiveModal('none'); setIsCalling(false); }} className="px-8 py-3 bg-red-100 text-red-600 font-bold rounded-full hover:bg-red-200 transition-colors flex items-center gap-2"><X size={18} /> Batalkan</button>
                    </div>
                  )}
                </div>
              )}
              ...
              {activeModal === 'journal' && (
                <div className="p-6 pt-10">
                  ...
                  <button onClick={() => { setActiveModal('none'); setJournalText(''); }} className="w-full py-3.5 mt-4 bg-primary text-white font-bold rounded-2xl hover:bg-primary/90 flex items-center justify-center gap-2"><PenTool size={18} /> Simpan Jurnal</button>
                </div>
              )}
                  ...
                  <div className="space-y-4">
                    {proDoctors.map((doc, i) => (
                      <div key={i} className="flex items-center gap-4 p-3 border border-border rounded-2xl bg-card">
                        ...
                        <button className="w-10 h-10 rounded-full bg-secondary text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-colors"><MessageCircle size={18} /></button>
                      </div>
                    ))}
                  </div>
                </div>
